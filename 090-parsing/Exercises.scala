...
trait Parsers[ParseError, Parser[+_]]:
  ...
  extension [A](p1: Parser[A])
    ...
    def *|(p2: => Parser[Any]): Parser[A] =
      (p1 ** p2).map { (a, _) => a }
    def ? : Parser[Option[A]] = { p1.map { Some(_) } } | succeed(None)
    def * : Parser[List[A]] =
      p1.many
  end extension
...
enum Result[+A]:
  ...
  /** Adjust the number of characters consumed while producing the result. `n`
    ...
    * the result (the total number of consumed. could be higher if other parsers
    * have already consumed some part of the input).
    */
...
object Sliceable extends Parsers[ParseError, Parser]:
  ...
  /** Returns -1 if s.startsWith(s2), otherwise returns the first index where
    * the two strings differed. If s2 is longer than s, returns s.length. Note:
    * locally imperative.
    */
