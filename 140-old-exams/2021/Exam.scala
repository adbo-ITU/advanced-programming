/** ****************************** Final Exam: Advanced Programming by Andrzej
  * Wąsowski IT University of Copenhagen, Autumn 2020: 10 January 2022, 15:00
  *
  ...
  * solutions, as well as standard library functions. You can access any written
  * or electronic material, also online, but you are not allowed to communicate
  * with anybody. By submitting, you legally declare to have solved the problems
  * alone, without communicating with anybody.
  *
  * Solve the tasks in the file 'Exam.scala' (this file) found in the zip
  ...
  * do compile, you can use the 'build.sbt' file provided in the zip archive
  * linked above. It has the necessary library dependencies configured, and the
  * source files from the semester are included.
  *
  * The percentage at the beginning of each question will be used as a soft
  * indicative weight in grading.
  *
  * Good luck!
  */
...
object Q1:
  ...
  /** * WARM-UP
    ...
    *
    * abstract class Printable: def hello(): String
    *
    * class Triangle extends Printable: override def hello(): String =
    * "triangle"
    *
...
object Q2:
  ...
  /** * SEQUENCE AND EITHER
    ...
    * with `sequence` for Option: at least one failure (Left) in the input list
    * should result in a failure overall, a single Left. Otherwise return the
    * Right values from the input list wrapped in a single Right. For full
    * points make sure that the error value returned, if any, is the *last*
...
/** * TYPE EXTENSIONS
  ...
  * Q5. (5%)
  *
  * Make all functions of RNG.Rand available for State.Rand by a suitable type
  * conversion:
  */
...
object Q8:
  ...
  /** ** A BETTER SIZE FOR LAZY STREAMS
    ...
    * Q8. (5%)
    *
    * Implement a pure function `checkIfLongerEqThan` that checks whether a
    * stream is longer or equal than a given bound. Do not use the standard
    * library functions lengthCompare, sizeCompare, lenghtIs, or sizeIs. You can
...
object Q9:
  ...
  /** * TESTING
    *
    * Assume we have a solution for question Q8 (even if you skipped it), so
    ...
    * Q9. (10%)
    *
    * Use it to write a property-based test that checks if the size of every
    * nonEmpty stream concatenated with itself is larger or equal than 2.
    */
...
/** * Par[Option[_]] vs Option[Par[_]]
  ...
  * Q11. (5%)
  *
  * Explain in English what the function from Q10 achieves. Provide its user
...
object Q12:
  ...
  given idMonad: Monad[Id] = new:
    ...
    extension [A](a: Id[A]) override def flatMap[B](f: A => Id[B]): Id[B] = f(a)
